...
def extract_csi_by_label(raw_folder, label, labels, save=False, win_len=300, thrshd=0.6, step=50):
    ...
    if label not in labels:
        raise ValueError("The label {} should be among 'lie down', 'fall', 'bend', 'run', 'sitdown', 'standup', 'walk'".format(label))
    ...
    # if not feature:  # Check if feature list is empty
    #     print(f"No features extracted for label: {label}")
    #     return np.empty((0, win_len, 52)), np.empty((0, len(labels)))  # Return empty arrays
    if not feature:  # Check if the feature list is empty
        print(f"No features extracted for label: {label}")
        return np.array([]), np.array([])  # Return empty arrays for x and y
...
def train_valid_split(numpy_tuple, train_portion=0.8, seed=200):
    ...
    if len(numpy_tuple) == 0 or any(arr.size == 0 for arr in numpy_tuple):
        raise ValueError    ("One or more input arrays are empty. Please check your data extraction.")
...
numpy_tuple = cfg.preprocessing('CSI-HAR-Dataset', save=True)
